...
This package converts the current manual workflow into a repeatable, auditable LinkedIn job-intelligence scraper.
...
• `CLIENT_BRIEF.md` — client profile + weighted skills + exclusions (legacy alias: `USER.md`)  
• `AGENCY.md` + `STYLE_GUIDE.md` — agency behavior, tone, and report standards (legacy alias: `SOUL.md`)  
• `AGENTS.md` — multi-agent state + handoff protocol  
• `CASE_LOG.md` — long-term case stats + blacklist/whitelist + run learnings (legacy alias: `MEMORY.md`)  
